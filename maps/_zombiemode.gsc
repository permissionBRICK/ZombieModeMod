...
main(init_zombie_spawner_name)
{
	...
	//AssertEx(level.script == "nazi_zombie_factory"||level.script == "nazi_zombie_htl", "This mod does only support: -Der Riese-, please play no other maps with this mod!");
...
//	level thread coop_player_spawn_placement();
	...
	{
		players[i] thread check_standing_still();
	}
}
...
track_players_ammo_count()
{
	...
	while(1)
	{
		...
		for(i=0;i<players.size;i++)
		{
			if(players[i] maps\_zombiemode_utility::akriteria())
			{
			players[i].adpl=true;
			}
			else
			{
			players[i].adpl=false;
			}
			...
			if(!isDefined(weap) || weap == "none" || weap == "zombie_perk_bottle_doubletap" || weap == "zombie_perk_bottle_jugg" || weap == "zombie_perk_bottle_revive" || weap == "zombie_perk_bottle_sleight" || weap == "mine_bouncing_betty" || weap == "syrette" || weap == "zombie_knuckle_crack" || weap == "zombie_bowie_flourish"|| weap == level.hacker )
			{
				continue;
			}
...
init_sounds()
{
	...
	add_sound( "Star_sound", "Star_sound" );
	add_sound( "weap_type92_act", "weap_type92_act" );
}
...
player.regenammo=false;
player.canselfrevive=false;
		player.is_zombie = false; 
		player.initialized = false;
		player.cantdamageself=false;
...
#/
	...
	//max = 1;
	old_spawn = undefined;
	if(IsDefined(level.callboss)&&level.callboss==true)
{
level.backup_zombie_move_speed=level.zombie_move_speed;
level.backup_zombie_health=level.zombie_health;
...
{
level.zombie_move_speed = (level.round_number-14) * 10;
}
	level.zombie_health = level.zombie_vars["zombie_health_start"]*(100+(level.round_number*10)); 
	if(level.round_number<15)
    {
	max=1;
	}
	...
	{
	max=(level.round_number-13)*2;
	}
level.zombie_total=max;

level.callboss=false;
level.calledboss=true;
	...
	//	iPrintLn(spawn_point.targetname + " " + level.zombie_vars["zombie_spawn_delay"]);
	    if((!IsDefined(level.zombiehorde))||(!level.zombiehorde==true))
		{
		...
		if ( IsDefined( level.mixed_rounds_enabled ) && level.mixed_rounds_enabled == 1&&level.flytrap_counter<3 )
		{
			spawn_dog = false;
		...
		if(IsDefined(level.calledboss)&&level.calledboss==true)
		{
			ai.isboss=true;
		}
		if((!IsDefined(level.zombiehorde))||(!level.zombiehorde==true))
	...
	if( level.round_number > 3&&!(IsDefined(level.calledboss)&&level.calledboss==true) )
	{
...
if(IsDefined(level.calledboss)&&level.calledboss==true)
{
level.calledboss=false;
level.zombie_move_speed=level.backup_zombie_move_speed;
level.zombie_health=level.backup_zombie_health;
}
}
...
award_grenades_for_survivors()
{
	...
	for (i = 0; i < players.size; i++)
	{
		if (!players[i].is_zombie)
		{
			...
			if(players[i].regenammo==true&&players[i] HasWeapon( "zombie_cymbal_monkey" ) )
			{
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	{
		if(eAttacker==self&&self.cantdamageself==true)
		{
			iDamage=0;
			//self maps\_zombiemode_weapons::WriteScreenText("Selbst Damage Abgewehrt!!");
			return;
		}
...
can_hack( object,dist )
{
	if(!IsDefined(dist))
	{
		dist=false;
	}
	if(self getcurrentweapon()!=level.hacker)
		return false;
		
	if ( !self is_facing( object ) )
		return false;
		
	if( !(SightTracePassed( self.origin + ( 0, 0, 50 ), object.origin + ( 0, 0, 30 ), false, undefined ) ||dist))		
		return false;
	//Distanz oder im trigger
	if(DistanceSquared(object.origin, self.origin)>50*50&&dist)
		return false;
		
	//chrisp - fix issue where guys can sometimes revive thru a wall	
	if(!(bullettracepassed(self.origin + (0,0,50), object.origin + ( 0, 0, 30 ), false, undefined)||dist) )
	{
		return false;
	}
...
is_hacking( object,dist )
{	
	if(!IsDefined(dist))
	{
		dist=false;
	}
	return ( self can_hack( object ,dist ) && self UseButtonPressed() );
}
...
hack(object,hackTime,dist)
{
	if(!IsDefined(dist))
	{
		dist=false;
	}
	if(!self is_hacking( object ))
	{
		return false;
	}
	//Hacken-Display
	if( !isdefined(self.hackerProgressBar) )
		self.hackerProgressBar = self createPrimaryProgressBar();

	if( !isdefined(self.hackerTextHud) )
		self.hackerTextHud = newclientHudElem( self );	
	
	
	self.hackerProgressBar updateBar( 0.01, 1 / hackTime );
	...
	self.hackerTextHud setText( "Hacking..." );
	hacked = false;
	timer=0;
	while( self is_hacking( object,dist ) )
	{
		wait( 0.05 );					
		timer += 0.05;			
	...
	//Hacken-Display Entfernen
	if( isdefined( self.hackerProgressBar ) )
	{
		self.hackerProgressBar destroyElem();
	}
...
// maps\_zombiemode::can_hack()
// maps\_zombiemode::hack()