...
tesla_damage_init( hit_location, hit_origin, player )
{
	...
	if(IsDefined(self.damageweapon)&&self.damageweapon=="zombie_kar98k_upgraded")
	{
		self kark_tesla_arc_damage( self, player, 1 );
	}
	else
	{
...
// kark_tesla_dmg funktion
kark_tesla_arc_damage( source_enemy, player, arc_num )
{
	player endon( "disconnect" );
	...
	enemies = kark_tesla_get_enemies_in_area( self GetTagOrigin( "j_head" ), 10000, player );
	...
	for( i = 0; i < enemies.size; i++ )
	{
		...
		enemies[i] kark_tesla_arc_damage( self, player, arc_num + 1 );
	}
}
...
//Kark funktion
kark_tesla_get_enemies_in_area( origin, distance, player )
{
	/#
	...
	if ( IsDefined( zombies ) )
	{
		for ( i = 0; i < zombies.size; i++ )
		{
			...
			if ( DistanceSquared( origin, test_origin ) > distance_squared &&level.supertesla!=true)
			{
				continue;
			}
			count++;
...
tesla_get_enemies_in_area( origin, distance, player )
{
	...
	if ( IsDefined( zombies ) )
	{
		for ( i = 0; i < zombies.size; i++ )
		{
			...
			if ( !BulletTracePassed( origin, test_origin, false, undefined ) &&level.supertesla!=true)
			{
				continue;
			}
...
tesla_do_damage( source_enemy, arc_num, player )
{
	...
	if(self.health> 5000 +self.health/5)
	{
		if(IsDefined(self.isboss)&&self.isboss==true)
		{
			self DoDamage(5000 +self.health/5, origin, player );
			wait(1);
			tesla_flag_hit(self,false);
			return;
		}
	}
...
tesla_play_death_fx( arc_num )
{
	...
	if ( !self enemy_is_dog() )
	{
		if( RandomInt( 100 ) < level.zombie_vars["tesla_head_gib_chance"] && !(IsDefined(self.isboss)&&self.isboss==true) )
		{
...
tesla_play_arc_fx( target )
{
	...
	if(IsDefined(self.isateddy)&&self.isateddy==true)
	{
	tag="tag_origin";
	}
...
is_tesla_damage( mod )
{
	return ( ( IsDefined( self.damageweapon ) && (self.damageweapon == "zombie_kar98k_upgraded" || self.damageweapon == "tesla_gun" || self.damageweapon == "tesla_gun_upgraded" ) ) && ( mod == "MOD_PROJECTILE" || mod == "MOD_PROJECTILE_SPLASH" ) );
}
