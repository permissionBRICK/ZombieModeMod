...
vending_upgrade()
{
	...
	for( ;; )
	{
		...
		if (player maps\_laststand::player_is_in_laststand()||(IsDefined(player.haslaserpower)&&player.haslaserpower==true) )
		{
			wait( 0.1 );
			continue;
		}
		...
		if( !IsDefined( level.zombie_include_weapons[current_weapon] ) || !IsDefined( level.zombie_include_weapons[current_weapon + "_upgraded"] )||current_weapon==level.hacker )
		{
			continue;
		}
...
wait_for_player_to_take( player, weapon, packa_timer )
{
	...
	while( true )
	{
		...
		{
			if( !player maps\_laststand::player_is_in_laststand()&&(!IsDefined(player.haslaserpower)||player.haslaserpower==false) )
			{
				self notify( "pap_taken" );
				primaries = player GetWeaponsListPrimaries();
				maxallowed=2;
				if(IsDefined(player.hasthirdweapon)&&player.hasthirdweapon==true)
				{
					maxallowed=3;
				}
				if( isDefined( primaries ) && primaries.size >= maxallowed )
				{
...
vending_trigger_think()
{
	...
	for( ;; )
	{
		...
		case "specialty_armorvest":
			cost = 2500;
			if(player HasPerk( "specialty_armorvest" )){cost = 5000;}
			break;

		case "specialty_quickrevive":
			cost = 1500;
			if(player HasPerk( "specialty_quickrevive" )){cost = 3000;}
			break;

		case "specialty_fastreload":
			cost = 3000;
			if(player HasPerk( "specialty_fastreload" )){cost = 4500;}
			break;

		case "specialty_rof":
			cost = 2000;
			if(player HasPerk( "specialty_rof" )){cost = 4000;}
			break;
		...
		if ( player HasPerk( perk ) )
		{
			...
			if ((perk=="specialty_rof"&&player.regenammo==true)||(perk=="specialty_quickrevive"&&player.canselfrevive==true)||(perk=="specialty_armorvest"&&player.cantdamageself==true) )
			{
			...
			if(perk=="specialty_fastreload"&& IsDefined(player.superspeed) &&player.superspeed==true)
			{
			//player iprintln( "Already using Perk: " + perk );
				self playsound("deny");
				player thread play_no_money_perk_dialog();
			
				self maps\_zombiemode_weapons::WriteScreenText(" " + perk + "    " +player.superspeed);
				continue;
			}
		...
		if(perk=="specialty_quickrevive"&&player HasPerk( "specialty_quickrevive" ))
		{
		player.canselfrevive=true;
		player thread perk_vo(perk);
		player setblur( 4, 0.1 );
		wait(0.1);
		player setblur(0, 0.1);
		}
		else if(perk=="specialty_rof"&&player HasPerk( "specialty_rof" ))
		{
		player.regenammo=true;
		...
		else if(perk=="specialty_fastreload"&&player HasPerk( "specialty_fastreload" ))
		{
		player.superspeed=true;
		player thread setmovespeed();
		player thread perk_vo(perk);
		player setblur( 4, 0.1 );
		wait(0.1);
		player setblur(0, 0.1);
		}
		else if(perk=="specialty_armorvest"&&player HasPerk( "specialty_armorvest" ))
		{
		//self maps\_zombiemode_weapons::WriteScreenText("Doppel Jugger Gekauft!");
		player.cantdamageself=true;
		player thread perk_vo(perk);
		player setblur( 4, 0.1 );
		wait(0.1);
		player setblur(0, 0.1);
...
if(player hasperk("specialty_armorvest")==true&&player hasperk("specialty_quickrevive")==true&&player hasperk("specialty_fastreload")==true&&player hasperk("specialty_rof")==true)
{
player achievement_notify("DLC3_ZOMBIE_ALL_PERKS");
}
if(player.cantdamageself==true&&player.superspeed==true&&player.regenammo==true&&player.canselfrevive==true)
{
player achievement_notify("DLC3_ZOMBIE_ALL_PERKS_TWICE");
}
...
give_ammo()
{
	for(;;)
	{
		...
		//self maps\_zombiemode_weapons::WriteScreenText("Waffenname: "+weapon);
		if ((weapon == "m7_launcher_zombie") ||(weapon == "panzerschrek_zombie") || (weapon == "tesla_gun"))
		{
			wait(10);
		}
		else if ((weapon == "zombie_doublebarrel") ||(weapon == "zombie_doublebarrel_sawed")||(weapon == "zombie_shotgun")||(weapon == "m7_launcher_zombie_upgraded")||(weapon == "tesla_gun_upgraded"))
		{
		   wait(5);
		}
		//Alle Pistolen haben 1 Schuss / 3 Sekunden (außer colt) und Sniper und upgeradete Schrots auch
		else if ((weapon == "zombie_sw_357")||(weapon == "ray_gun")||(weapon == "zombie_kar98k")||(weapon == "zombie_kar98k_upgraded")||(weapon == "zombie_springfield")||(weapon == "zombie_gewehr43")||(weapon == "zombie_m1carbine")||(weapon == "ptrs41_zombie")||(weapon == "ptrs41_zombie_upgraded") ||(weapon == "zombie_shotgun_upgraded")||(weapon == "zombie_doublebarrel_sawed_upgraded")||(weapon == "zombie_doublebarrel_upgraded"))
		{
		   wait(3);
		}
		//Colt, upgegradete nicht-repadierende Sniper
		else if ((weapon == "zombie_gewehr43_upgraded")||(weapon == "zombie_m1carbine_upgraded")||(weapon == "zombie_sw_357_upgraded")||(weapon == "ray_gun_upgraded"))
		{
			wait(1);
		}
		else if((weapon == "zombie_mg42_upgraded")||(weapon == "zombie_30cal_upgraded")||(weapon == "zombie_ppsh_upgraded"))
		{
			wait(0.15);
		}
		else if((weapon == "zombie_mg42")||(weapon == "zombie_30cal")||(weapon == "zombie_ppsh"))
		{
			wait(0.25);
		}
		else
		...
		weapon = self GetCurrentWeapon();
		if (!(weapon == "mine_bouncing_betty"||weapon=="zombie_kar98k_upgraded"||weapon=="weapon_kar98k"||self.moving==true)) 
		{
			self SetWeaponAmmoStock( self GetCurrentWeapon(), self GetAmmoCount(self GetCurrentWeapon())-self GetWeaponAmmoClip(self GetCurrentWeapon()) +1);
		}
...
#/
		...
		while(true)
		{
			...
			for( i = 0; i < players.size; i++ )
			{
				...
				{
					if(!players[i] hasperk(perk)||(perk=="specialty_rof"&&players[i].regenammo!=true)||(perk=="specialty_quickrevive"&&players[i].canselfrevive!=true)||self canbuyperk(players,perk,i))
					{
						if(!players[i] in_revive_trigger())
						{
...
canbuyperk(players,perk,i)
{
if((perk=="specialty_armorvest"&&players[i].cantdamageself!=true)||(perk=="specialty_fastreload" &&(players[i].superspeed!=true||!IsDefined(players[i].superspeed))))
{
return true;
}
else
...
self SetHintString( &"ZOMBIE_PERK_JUGGERNAUT" );
wait(3);
self SetHintString ("Press & hold F a second time to buy Self-Damage Invulnerability [Cost: 5000]");
wait(3);
}
}
ShowSpeedHint()
...
self SetHintString( &"ZOMBIE_PERK_FASTRELOAD" );
wait(3);
self SetHintString ("Press & hold F a second time to buy Fast Sprinting Soda [Cost: 4500]");
wait(3);
}
}
ShowDoubletapHint()
...
self SetHintString( &"ZOMBIE_PERK_DOUBLETAP" );
wait(3);
self SetHintString ("Press & hold F a second time to buy Ammo Regeneration [Cost: 4000]");
wait(3);
}
}
...
self SetHintString( &"ZOMBIE_PERK_QUICKREVIVE" );
wait(3);
self SetHintString ("Press & hold F a second time buy Continous Last Stand [Cost: 3000]");
wait(3);
}
}
...
{
self waittill_any( "fake_death", "death", "player_revived" );
...
{
if(player.stats["perks"]>3)
{
thread play_sound_2d( "sam_nospawn" );
//self maps\_zombiemode_weapons::WriteScreenText("Teddy gave you nothing...");
return;
}
while(1==1)
{
