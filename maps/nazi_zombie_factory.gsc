...
//	level.crawlers_enabled = true;		//MM - added for crawler support
	...
	SetDvar( "perk_altMeleeDamage", 10000 ); // adjusts how much melee damage a player with the perk will do, needs only be set once
...
{
wait(5);
iprintln("You are playing: Der UberRiese.");
wait(12);
iprintln( "This Mod was created by Crym1n4l!" );
wait(20);
iprintln( "Please check for updates at: uberriese.sourceforge.net" );
wait(120);
iprintln( "Like this Mod? Check out my other Work!" );
wait(120);
iprintln( "Please report any bugs you find!" );
wait(120);
if(RandomInt(10)==1)
{
iprintln( "...and thanks for reading this Spam!" );
}
}
...
init_achievement()
{
	...
	include_achievement( "achievement_power_on" );
	...
	include_achievement( "achievement_found_eggs");
include_achievement( "achievement_all_perks");
include_achievement("achievement_good_player");
}
...
init_sounds()
{
	...
	maps\_zombiemode_utility::add_sound( "packa_weap_ready","packa_weap_ready");
}
...
factory_ray_gun_weighting_func()
{
	...
	{	
		...
		if( isDefined( level.pulls_since_last_ray_gun ) )
		{
			...
			if( level.pulls_since_last_ray_gun > 13 )
			{
				num_to_add += int(level.zombie_include_weapons.size*0.2);
			}
			else if( level.pulls_since_last_ray_gun > 9 )
			{
				num_to_add += int(level.zombie_include_weapons.size*0.1);
			}			
			// after 8 pulls the Ray Gun percentage increases to 10%
			else if( level.pulls_since_last_ray_gun > 6 )
			{
...
include_powerups()
{
	...
	include_powerup( "Star" );
	include_powerup( "ray_gun" );
	include_powerup( "teddytrap" );
	include_powerup( "grenade" );
	include_powerup( "perk" );
	include_powerup( "jackpot" );
}
...
// 	}
	...
	level achievement_notify("DLC3_ZOMBIE_POWER_ON");
...
//	clientnotify( "power_on" );
	...
	while(1)
	{
		...
		if(user.adpl)
		{
			primaryWeapons = user GetWeaponsList(); 

			for( x = 0; x < primaryWeapons.size; x++ )
			{
				user GiveMaxAmmo( primaryWeapons[x] );
			}
		}
	}
}
...
{	
	...
	while(1)
	{
		...
		{
			if( who.score >= self.zombie_cost||(IsDefined(who.freetraptickets)&&who.freetraptickets>=1) )
			{				
				if(!self.zombie_dmg_trig.in_use)
				{
					...
					//set the score
					if(IsDefined(who.freetraptickets)&&who.freetraptickets>=1)
					{
						who.freetraptickets-=1;
					}
					else
					{
...
{	
	while(1)
	{
		...
		{
			if(!(IsDefined(ent.isboss)&&ent.isboss==true))
			{
...
flytrap_samantha_vox()
{
	...
	if( level.flytrap_counter == 3 )
	{
		...
		level achievement_notify("DLC3_ZOMBIE_FOUND_EGGS");
		return;
	}
	wait(0.05);
}
