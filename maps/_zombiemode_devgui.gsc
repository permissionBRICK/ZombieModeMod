...
zombie_devgui_think()
{
	for ( ;; )
	{
		...
		case "tesla_gun":
			...
			break;
			case "teddy":
			if(IsDefined(level.onlyteddy)&&level.onlyteddy==true)
			{
			level maps\_zombiemode_weapons::WriteScreenText("Teddymode Off");
			level.onlyteddy = false;
			}
			else
			{
			level maps\_zombiemode_weapons::WriteScreenText("Teddymode On");
			level.onlyteddy = true;
			}
			break;
		case "teddymode":
			level.teddymode=GetDvarInt( "zombie_teddymode");
			text="Teddymode set to "+level.teddymode;
			level maps\_zombiemode_weapons::WriteScreenText(text);
			break;
			case "tesla_power":
