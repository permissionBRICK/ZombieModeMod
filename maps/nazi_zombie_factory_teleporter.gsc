...
//	self teleport_trigger_invisible( false );
	...
	while ( 1 )
	{
		...
		if ( is_player_valid( user ) && (user.score >= level.teleport_cost||(IsDefined(user.freeteleticket)&&user.freeteleticket>=1)) && !level.is_cooldown )
		{
			for ( i = 0; i < level.teleporter_pad_trig.size; i++ )
			{
				level.teleporter_pad_trig[i] teleport_trigger_invisible( true );
			}
if(IsDefined(user.freeteleticket)&&user.freeteleticket>=1)
{
	user.freeteleticket-=1;
}
else
{
...
self endon( "fx_done" );
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < players.size; i++ )
		{
			if ( isdefined( players[i] ) )
			{
				if ( self check_close_player( players[i] ) )
				{
					players[i] SetTransported( 7 );
				}
				else
				{
...
zombies = getaispeciesarray("axis");
	...
	zombies = get_array_of_closest( self.origin, zombies, undefined, 20, 150 );
...
stareffect()
{
self thread [[ level.teleport_ae_funcs[int(5)] ]]();
}
