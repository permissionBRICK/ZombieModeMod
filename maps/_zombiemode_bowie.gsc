...
bowie_init()
{
	...
	for( i = 0; i < bowie_triggers.size; i++ )
	{
		...
		bowie_triggers[i].everyone_has_bowie=false;
	}
}
	...
	for( ;; )
	{
		...
		if( !player_has_bowie )
		{
			...
			if( player.score >= cost )
			{
				...
				player give_bowie(); 
				self.everyone_has_bowie=true;
					players=get_players();
					for(a=0;a<players.size;a++)
					{
					if(!players[a] HasPerk( "specialty_altmelee" ))
					{
					self.everyone_has_bowie=false;
					}
					}
					if(self.everyone_has_bowie==true)
					{
					self SetHintString("Press F to save up a Monkey for Last Stand!");
					}
			}
			else
			{
		...
		{
		if(maps\_zombiemode_weapons::has_weapon_or_upgrade("zombie_cymbal_monkey")&&player.hassavedmonkey!=true&&self.everyone_has_bowie==true)
		{
		weapon="zombie_cymbal_monkey";
		if(player GetWeaponAmmoClip( weapon ) >0)
		{
		if( player.score >= 1500 )
		{
		player SetWeaponAmmoClip( weapon, player GetWeaponAmmoClip( weapon ) -1 );
		player.hassavedmonkey=true;
