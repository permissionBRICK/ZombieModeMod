...
door_think()
{
	...
	while( 1 )
	{
		...
		else
		{
			...
			{
				hacked=false;
				if(who maps\_zombiemode::can_hack(self)&&who.score >= 200)
				{
					who maps\_zombiemode_score::minus_to_player_score( 200 );
					success = who maps\_zombiemode::hack(self,15);
					if(!success)
					{
						continue;
					}
					...
					if( who.score >= self.zombie_cost||hacked )
					{
						// set the score
						if(!hacked)
...
debris_think()
{
	...
	while( 1 )
	{
		...
		if( is_player_valid( who ) )
		{
			...
			{
				who maps\_zombiemode_score::minus_to_player_score( 200 );
				success = who maps\_zombiemode::hack(self,20);
				if(!success)
				{
					continue;
				}
			...
			if( who.score >= self.zombie_cost||free )
			{
				// set the score
				if(!free)
					who maps\_zombiemode_score::minus_to_player_score( self.zombie_cost ); 
...
blocker_trigger_think()
{
	...
	while( 1 )
	{
		...
		hacked = false;
		if(player maps\_zombiemode::can_hack(self))
		{
			success = player maps\_zombiemode::hack(self,2);
			if(!success)
			{
				continue;
			}
		...
		if(((IsDefined(player.freeinstawindows)&&player.freeinstawindows>=1)||hacked)&&player UseButtonPressed()&&!all_chunks_intact( self.barrier_chunks ))
		{
		if(!hacked)
			player.freeinstawindows-=1;
		while(1)
		{
			if( all_chunks_intact( self.barrier_chunks ) )
			{
			...
			if( !IsDefined( chunk ) )
				break;
			self thread maps\_zombiemode_blockers_new::replace_chunk( chunk, false, true );
		...
		play_sound_at_pos( sound, self.origin );
		}
		else
		{
		while( 1 )
...
RepairWindow()
{
self.stoprepair=false;
self thread Repair_Stop_Timer();
while(!self.stoprepair)
		{
			if( all_chunks_intact( self.barrier_chunks ) )
			{
				wait(4);
