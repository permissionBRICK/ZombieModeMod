...
//	self.meleeRangeSq = anim.meleeRange * anim.meleeRange; 
	if(!IsDefined(level.calledboss)||level.calledboss==false)
	{
...
zombie_hacklogic()
{
	...
	while(1)
	{
		...
		{
			if(players[i] maps\_zombiemode::is_hacking(self,true))
			{
				success = players[i] maps\_zombiemode::hack(self,3,true);
				if(success)
				{
					self thread create_zombie_point_of_interest( 500, 20, 1000 );
					tag = "J_SpineUpper";
					if ( self enemy_is_dog() )
					{
						tag = "J_Spine1";
					}
					location = self GetTagOrigin(tag);
					//network_safe_play_fx_on_tag( "tesla_death_fx", 2, level._effect["tesla_shock"], self, tag );
					self playsound( "imp_tesla" );
					thread network_safe_play_fx_on_tag( "tesla_death_fx", 2, level._effect["tesla_shock_eyes"], self, "J_Eyeball_LE" );
					wait(2);
					thread network_safe_play_fx_on_tag( "tesla_death_fx", 2, level._effect["tesla_shock_eyes"], self, "J_Eyeball_LE" );
					wait(2);
					thread network_safe_play_fx_on_tag( "tesla_death_fx", 2, level._effect["tesla_shock_eyes"], self, "J_Eyeball_LE" );
					...
					wait(1);
					location = self GetTagOrigin(tag);
					//playfx(level._effect["betty_explode"], location);
					//earthquake(1, .4, self.origin, 512);
					//maps\_zombiemode_weapons::AddExplosion(players[i],location);
					//maps\_zombiemode_weapons::AddExplosion(players[i],location);
					//maps\_zombiemode_weapons::AddExplosion(players[i],location);
					self thread maps\_zombiemode_tesla::tesla_damage_init( location, location, players[i] );
				}
			}
		}
		wait(0.1);
...
zombie_goto_entrance( node, endon_bad_path )
{
	...
	else// Guy should get to goal and tear into building until all barrier chunks are gone
	{*/
